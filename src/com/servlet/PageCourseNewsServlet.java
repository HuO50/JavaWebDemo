...
@WebServlet("/PageCourseNewsServlet")
public class PageCourseNewsServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

    public PageCourseNewsServlet() {
        super();
    }
    
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {

		System.out.println("精品课程内容如下");
		IResultService iResultService = new ResultService();
		List<News> news = iResultService.getKindResult("精品课程");
		...
		request.getRequestDispatcher("WEB-INF/jsp/PageCourseNews.jsp").forward(request, response);
