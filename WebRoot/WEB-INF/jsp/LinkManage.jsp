	...
	<div class="breadcrumb">
		<a class=" r mr-20" style="padding-top: 6px"
			...
			<span class="l">
				...
				onclick="article_add('新增链接','LinkAdd')" href="javascript:;">
					<i class="fa fa-plus"></i> 新增链接
				</a>
			</span> 
			<span class="r">共有数据：<strong>${fn:length(linklist)}</strong>条
			</span>
		</div>
	<div class="pd-20">
		<div class="mt-20">
				...
				<tbody>
					<c:forEach var="link" items="${linklist}">
						<tr class="text-c">
							...
							<td width="30%" style="text-align: left;" class="text-c">
								<a style="text-decoration: none; cursor: pointer; color: black;" 
                        		href="javascript:void(0)" 
                        		onclick="layer_show('${link.name }','LinkDetail?linkid=${link.id}','800','800')">
                       			${link.name }</a>
                       		</td>
                       		<td width="30%">${link.link }</td>
							<td class="f-14 td-manage">
								 <a style="text-decoration:none"
								class="ml-5" onClick="article_del('删除链接','LinkDeleteResult?linkid=${link.id}')"
