			...
			<div class="aside">
				<ul>
					<a href="PageCommunicationNews">
						<li class="btn chooseContent" ng-click="changeContent($event,4,1)"
							...
							<c:forEach var="news1" items="${news1}">
								<tr class="text-l">
									<td width="80%" class=""><c:if
											test="${news1.istop == true}">
											<font size="2" color="red">置顶</font>
										</c:if> &nbsp <a href="PageNews?id=${news1.id}"> <c:if
												test="${fn:length(news1.title)>'31'}">  
                   							 		${fn:substring(news1.title,0,31)}...  
           					 					</c:if> <c:if test="${fn:length(news1.title)<='31'}">  
               							 			${news1.title}  
            								</c:if>
									</a></td>
									<td width="20%" class="text-c"><span class="text-c">${news1.time}</span></td>
									<td style="display: none;">${news1.sortid }</td>
								</tr>
							</c:forEach>
						</tbody>
					</table>
