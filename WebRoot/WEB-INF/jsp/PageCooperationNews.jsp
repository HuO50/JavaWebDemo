...
<body ng-controller="mainController">
	<div class="wrap">
		...
		<div class="main">
			<div class="aside">
				<ul>
					<a href="PageCommunicationNews">
						<li class="btn" ng-click="changeContent($event,4,1)"
						ng-class="{'chooseContent':showContent == 2}"><span>学术交流</span></li>
					</a>
					<a href="PageCooperationNews">
						<li class="btn chooseContent" ng-click="changeContent($event,4,2)"
						ng-class="{'chooseContent':showContent == 1}"><span>合作研究</span></li>
					</a>
				</ul>
			</div>
			<div class="section">
								...
								<c:forEach var="news2" items="${news2}">
									<tr class="text-l">
										<td width="80%" class=""><c:if
												test="${news2.istop == true}">
												<font size="2" color="red">置顶</font>
											</c:if> &nbsp <a href="PageNews?id=${news2.id}"> <c:if
													test="${fn:length(news2.title)>'31'}">  
                   							 		${fn:substring(news2.title,0,31)}...  
           					 					</c:if> <c:if test="${fn:length(news2.title)<='31'}">  
               							 			${news2.title}  
            								</c:if>
										</a></td>
										<td width="20%" class="text-c"><span class="text-c">${news2.time}</span></td>
										<td style="display: none;">${news2.sortid }</td>
									</tr>
								</c:forEach>
							</tbody>
						</table>
